 ...
 * expMail is an integrator class, bringing the flexibility of SwiftMail into Tienda gracefully.
...
//					break;
		...
		switch (SMTP_DEBUGGING) {
			case 1:
				//To use the eDebugLogger for Tienda
...
//		$this->message->setFrom((array)$params['from']);  //FIXME we need to use this->addFrom() instead
		...
		$this->message->setSubject($params['subject'] = !empty($params['subject']) ? $params['subject'] : 'Order Confirmation from '.SITE_TITLE);
	...
	/**
	 *  send() - This is the main method for sending email messages.If there is not a from address, it defaults to the one stored in the Tienda config.
	 *
