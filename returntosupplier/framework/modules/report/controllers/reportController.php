...
//            'states'=>$states,
    ...
    function generateOrderReport() {
        ...
        $sql = "o.invoice_id, FROM_UNIXTIME(o.purchased,'%c/%e/%y %h:%i:%s %p') as purchased_date, b.firstname as bfirst, b.lastname as blast, concat('".expCore::getCurrencySymbol()."',format(o.grand_total,2)) as grand_total, o.total as total, os.title as status_title, ot.title as order_type";
...
//        }
    ...
    function batch_export() {
        ...
        $out = '"id","parent_id","rank","title","capacity","body","model","warehouse_location","sef_url","canonical","meta_title","meta_keywords","meta_description","tax_class_id","quantity","availability_type","base_price","special_price","use_special_price","active_type","product_status_id","category1","category2","category3","category4","category5","category6","category7","category8","category9","category10","category11","category12","surcharge","category_rank","feed_title","feed_body","child_rank", "manufacturing_price"' . chr(13) . chr(10);
        if (isset($this->params['applytoall']) && $this->params['applytoall'] == 1) {
        ...
        foreach ($prods as $pid) {
			...
			$out .= $this->outputField($p->rank);
            $out .= $this->outputField($p->title);
			$out .= $this->outputField($p->capacity);
			...
			$out .= $this->outputField($p->feed_body);
			$out .= $this->outputField($p->child_rank);
            $out .= substr($this->outputField($p->manufacturing_price), 0, -1) . chr(13) . chr(10); //Removed the extra "," in the last element
