...
//        'events_calendar'=>'Show events in a calendar'
    ...
    static function description() {
        return gt("Use this module to import and export data from your Tienda website.");
    }
    ...
    function importProduct($file) {
        ...
        */
        ...
        while (($data = fgetcsv($handle, 10000, ",")) !== FALSE) {
			...
			$product->capacity = stripslashes(stripslashes($data[4]));
            $product->body = utf8_encode(stripslashes($data[5]));         
            $product->model = stripslashes(stripslashes($data[6]));
            $product->warehouse_location = stripslashes(stripslashes($data[7]));
			$product->sef_url = $router->encode(stripslashes(stripslashes($data[3])) . " " . $data[4]);
			$product->canonical = stripslashes(stripslashes($canonical[9]));
            $product->meta_title = stripslashes(stripslashes($data[10]));
            $product->meta_keywords = stripslashes(stripslashes($data[11]));
            $product->meta_description = stripslashes(stripslashes($data[12]));

            $product->tax_class_id = $data[13];

            $product->quantity = $data[14];

            $product->availability_type = $data[15];

            $product->base_price = $data[16];
            $product->special_price = $data[17];
            $product->use_special_price = $data[18];
            $product->active_type = $data[19];
            $product->product_status_id = $data[20];

            $product->surcharge = $data[33];
            $product->feed_title = stripslashes(stripslashes($data[35]));
            $product->feed_body = stripslashes(stripslashes($data[36]));
			$product->child_rank = stripslashes(stripslashes($data[37]));
