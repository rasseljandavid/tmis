...
//                }
	...
	function addItemListing() {
		...
		$res = $product->addToCart($this->params);
	...
	function removeItemListing() {
        global $order, $db;
        foreach ($order->orderitem as $item) {
            if ($item->product_id == intval($this->params['product_id'])) {
                $res = $db->delete("orderitems", "id = {$item->id}");
				break;
            }
        }
	...
	function increaseItemListing() {
	     ...
	     if($res) {
			...
			$product = new product($this->params['product_id']);
			$res = $product->addToCart($this->params,  $this->params['quantity']);
...
//            'cartConfig'          => $config->config,
    ...
    public function preprocess() {
        ...
        global $order, $user, $db;
				...
				if(empty($this->params['delivery_date']) && empty($order->delivery_date)) { 
				 flash('error', "Please Select a Delivery Date.");
		         expHistory::back();
			}

		if(empty($this->params['delivery_slot']) && empty($order->special_note)) { 
			 flash('error', "Please Select a Delivery Time Range.");
	         expHistory::back();
		}
		
	
			
		if(!empty($this->params['delivery_date']) != "" && !empty($this->params['delivery_slot'])) {
			$order_params['delivery_date'] = $this->params['delivery_date'];
			$order_params['special_note'] = $this->params['delivery_slot'];
...
//        }
        ...
        flash('message', "Thank you for using Tienda. <br />Please expect your order on " . $order->delivery_date . " between " . $order->special_note . '. Thanks!');
