			...
			<tbody>
				{foreach from=$page->records item=listing name=listings}
                    <tr class="{cycle values="odd,even"}">
						...
						<td>{($listing->totality - $od->computeProfit($listing->id))|currency}</td>
						<td>{$od->computeProfit($listing->id)|currency}</td>
                   		<td><a href="{link action=delete id=$listing->id}" class="icon-remove" onclick="return confirm('Are you sure you want to delete this order?');"></a></td>
 					</tr>
				{foreachelse}
				    <tr class="{cycle values="odd,even"}">
