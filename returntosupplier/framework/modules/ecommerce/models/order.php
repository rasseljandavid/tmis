...
public function computeCogs() {
	global $db;
	$total = 0;
	$oi = $db->selectObjects("orderitems", "orders_id IN (SELECT id FROM exponent_orders WHERE purchased != 0)");
	foreach($oi as $item) {
		$total +=$db->selectSum("orderitems", "(products_price - (products_price - manufacturing_price)) * quantity", "id = {$item->id}");
	}
	return $total;
}

	public function computeTotalProfit() {
		global $db;
		$total = 0;
		$oi = $db->selectObjects("orderitems", "orders_id IN (SELECT id FROM exponent_orders WHERE purchased != 0)");
		foreach($oi as $item) {
			$total +=$db->selectSum("orderitems", "(products_price - manufacturing_price) * quantity", "id = {$item->id}");
		}
		return $total;
	}

public function computeTotalOrder() {
		global $db;
		$total = 0;
		$oi = $db->selectObjects("orderitems", "orders_id IN (SELECT id FROM exponent_orders WHERE purchased != 0)");
		foreach($oi as $item) {
			$total +=$db->selectSum("orderitems", "(products_price * quantity)", "id = {$item->id}");
		}
		return $total;
	}
	...
	public function computeProfit($order_id = '') {
		global $db;
		
		return $db->selectSum("orderitems", "(products_price - manufacturing_price) * quantity", "orders_id = {$order_id}");
	}
