...
class product extends expRecord {
    ...
    public $quantity_discount_amount_modifiers = array('P' => 'P', '%' => '%');
...
//        'content_expTags'=>'expTag',
    ...
    function addToCart($params, $orderid = null) {
        ...
        //eDebug($this);
        //
        $params['qty'] = isset($params['quantity']) ? $params['quantity'] : 1;
	
        $params['product_type'] = 'product';

        $this->createOrderItem($this, $params, null, $orderid);
    ...
    private function createOrderItem($product, $params, $user_input_info, $orderid) {
        ...
        $item->manufacturing_price = $product->manufacturing_price;
...
//                    $child->model = str_ireplace($originalModel, $product->model, $child->model);
    ...
    public function addtomylist() {
		...
		if($user->id && $this->id) {
			$db->delete("mylists", "user_id = {$user->id} && product_id = {$this->id}");
