	...
	Y.Slideshow = function(pTarget, pConfig){
		...
		if(this.previousButton !== false){
			Y.on('click', function(){
        		...
        		Y.one(this.playButton).removeClass('hide');
		...
		if(this.playButton !== false){
			Y.on('click', function(){
        		this.paused = false;
        		Y.one(this.playButton).addClass('hide');
        		Y.one(this.pauseButton).removeClass('hide');
				//if(this.loop === false){
				this.slideChange('next');
				this.startLoop();
				//}
		...
		if(this.pagination !== false){
            var linkParent;
            if (Y.one(this.pagination)) {
                linkParent = Y.one(this.pagination).get('parentNode');
                linkParent.one('[rel="0"]').addClass('current');
            }
			Y.on('click', function(e){
			    e.halt();
        		if (!Y.Lang.isNull(Y.one(this.pauseButton))) Y.one(this.pauseButton).addClass('hide');
        		if (!Y.Lang.isNull(Y.one(this.playButton))) Y.one(this.playButton).removeClass('hide');
				if(this.loop !== false && this.stopOnUser === true){
					clearInterval(this.loop);
					this.loop = false;
				}
