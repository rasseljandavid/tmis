...
//        $this->manage_lang();
    ...
    public function toggle_minify() {
    	...
    	$message = (MINIFY != 1) ? gt("Tienda is now minifying Javascript and CSS") : gt("Tienda is no longer minifying Javascript and CSS") ;
	...
	public function toggle_dev() {
	    ...
	    expTheme::removeCss();
		$message = (DEVELOPMENT != 1) ? gt("Tienda is now in 'Development' mode") : gt("Tienda is no longer in 'Development' mode") ;
		flash('message',$message);
		expHistory::back();
	}
	...
	public function toggle_maintenance() {
		...
		MAINTENANCE_MODE == 1 ? flash('message',gt("Tienda is no longer in 'Maintenance' mode")) : "" ;
		expHistory::back();
	}

	public function toggle_preview() {eDebug($this->params, true);
		$level = 99;
		...
		$message = ($level == UILEVEL_PREVIEW) ? gt("Tienda is no longer in 'Preview' mode") : gt("Tienda is now in 'Preview' mode") ;
		flash('message',$message);
		expHistory::back();
	}
    ...
    public function manage_version() {
        ...
        if (!expVersion::checkVersion(true)) {
            flash('message', gt('Your version of Tienda CMS is current.'));
        }
   		expHistory::back();
   	}
...
//        $form->register('patch',gt('Patch Tienda CMS or Install Theme?'),new checkboxcontrol(false,false),null,null,gt('All extensions are normally placed within the CURRENT theme (folder)'));
...
//                finfo_close($finfo);
    ...
    public function theme_switch() {
        ...
        // $message = (MINIFY != 1) ? "Tienda is now minifying Javascript and CSS" : "Tienda is no longer minifying Javascript and CSS" ;
        // flash('message',$message);
...
//        }
   		...
   		exit('Redirecting to the Tienda Install Wizard');
   	}
