<?php
$clients = array();
$clients_raw = getCustomers();
foreach($clients_raw as $item) {
	if(strtolower(trim($item->SupplierClientComments)) == "cash" || strtolower(trim($item->SupplierClientComments)) == "a") {
		$clients[] = $item;
	}
}
...
foreach($clients as $client) {
	...
	$arr['query'] = "mv.DocumentSupplierClientID={$client->SupplierClientID}";
	...
	foreach($res->mvDocuments as $item) {
		if(trim($item->DocumentTypeAbbreviation) == "GOSI") {
		
			
			@$data[$client->SupplierClientName][convertDate($item->DocumentDate)]->total += $item->DocumentAmountGrandTotal;
			$data[$client->SupplierClientName][convertDate($item->DocumentDate)]->color  = $client->SupplierClientComments;
		}
	}
