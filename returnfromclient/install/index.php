...
//	flash('notice',gt('This Tienda Site has already been configured.'));
//	header('Location: ../index.php');
//	exit(gt('This Tienda Site has already been configured.'));
//}
...
//        echo gt('Sample content was added to your database.  This content should help you learn how Tienda works, and how to use it for your website.');
    }
}
...
switch ($page) {
	case 'upgrade-1':
	    $masthead = gt("Upgrade");
		$page_text = gt("It appears you've upgraded your Tienda code.").'<br /><br />'.gt("Before you begin the upgrade you should").' <a href="javascript:void(0)" onclick="return pop(\'changes\');">'.gt('read about the changes!').'</a> ';
		break;
    case 'upgrade-2':
        $masthead = gt("Upgrade");
   		$page_text = gt('Tienda requires that several file permissions be set correctly in order to operate.').' '.
            gt('Sanity checks are being run right now to ensure that the web server directory you wish to upgrade Tienda in, is suitable.').'<br><br>'.
	...
	case 'upgrade-3':
	    $masthead = gt("Upgrade");
		$page_text = gt("Installing Tables adds any new fields to existing tables, and adds any additional tables Tienda needs to be awesome.");
		break;
	case 'upgrade-4':
	    $masthead = gt("Upgrade");
		$page_text = gt("We'll now run any upgrade scripts needed for this version of Tienda.");
		break;
	case 'install-1':
        $masthead = gt("New Installation");
		$page_text = gt('Tienda requires that several file permissions be set correctly in order to operate.').' '.
            gt('Sanity checks are being run right now to ensure that the web server directory you wish to install Tienda in, is suitable.').'<br><br>'.
	        gt('If something fails, please').' <a href="javascript:void(0)" onclick="return pop(\'sanity\');">'.gt('read about each sanity check').'</a> '.
            gt('for an explanation of what exactly the installer is checking for, and how to fix it.');
		break;
	case 'install-2':
        $masthead = gt("New Installation");
		$page_text = gt('Tienda requires a database to store and manage content.').' '.
			gt('Simply create a database using your database tool of of choice, and fill in the information on this page.');
		break;
	case 'install-3':
        $masthead = gt("New Installation");
		$page_text = gt('Tienda is now checking to make sure that the database configuration information you provided is valid.');
		break;
	case 'install-4':
        $masthead = gt("New Installation");
	...
	default:
        $masthead = gt("New Installation");
		$page = 'welcome';
		$page_text = gt('This installation wizard will guide you step by step through the configuration and setup of your new Tienda-powered website.');
		break;
}
...
<html>
<head>
	<title><?php echo gt('Tienda Install Wizard'); ?></title>
