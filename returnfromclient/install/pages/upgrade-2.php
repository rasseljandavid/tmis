...
if ($errcount > 0) {
	...
	echo gt('The Tienda Install Wizard found some major problems with the server environment, which you must fix before you can continue.');
	
	if (ini_get('safe_mode') == true) {
		echo '<br /><br /><div style="font-weight: bold; color: red;">'.gt('SAFE MODE IS ENABLED.  You may encounter many strange errors unless you give the web server user ownership of ALL Tienda files.  On UNIX, this can be done with a "chown -R" command').'</div>';
	}
	?>
	<br /><br />
	...
	?><p><?php
	echo gt('The Tienda Install Wizard found some minor problems with the server environment, but you should be able to continue.');
	?></p><?php
	
	if (ini_get('safe_mode') == true) {
    	?><p class="important_message"><?php
    	echo gt('SAFE MODE IS ENABLED. You may encounter many strange errors unless you give the web server user ownership of ALL Tienda files. On UNIX, this can be done with a "chown -R" command');
    	?></p><?php
	}
	...
	?><p><?php
    echo gt('The Tienda Upgrade Wizard found no problems with the server environment.');
    ?>
    </p>
    <p>
    <?php
    echo gt("Next, we'll").' <a href="http://docs.exponentcms.org/docs/current/update-tables" target="_blank">'.
...
if ($errcount == 0) {
?>
    <a class="awesome large green" href="index.php?page=upgrade-3"><?php echo gt('Continue to Install Tables'); ?></a>
<?php
}
