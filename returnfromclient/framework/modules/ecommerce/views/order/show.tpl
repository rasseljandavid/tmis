...
{css unique="showorder" link="`$asset_path`css/ecom.css" corecss="tables,button"}
...
<div id="order" class="module order show">
    <div id="ordertabs" class="yui-navset exp-skin-tabview hide">
        <ul class="yui-nav">
            <li class="selected"><a href="#invoice2"><em>{'Receipt'|gettext}</em></a></li>
        ...
        <div class="yui-content">
            <div id="invoice2">
                <div id="buttons">
                    {printer_friendly_link class="submit btn btn-primary btn-medium" text="Print this invoice"|gettext view="show_printable" show=1}
                    {if $smarty.const.HTMLTOPDF_PATH && $smarty.const.HTMLTOPDF_PATH_TMP} {* FIXME file_exists($smarty.const.BASE.'external/dompdf/dompdf.php'*}
                        <a class="awesome {$smarty.const.BTN_SIZE} orange" href="{link controller='order' action='getPDF' id=$order->id inum=$order->invoice_number}">{'Download PDF'|gettext}</a>
                    {/if}
                    {permissions}
                        {if $permissions.manage == 1}
                            ...
                            <a class="awesome {$smarty.const.BTN_SIZE} orange" href="{link controller='order' action='createReferenceOrder' id=$order->id}">{'Spawn Reference Order'|gettext}</a>
                        {/if}
                    {/permissions} 
                </div>               
