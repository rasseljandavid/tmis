...
//        global $user;
	...
	public static function headerInfo($config) {
		...
		$str .= "\t".'<meta name="Generator" content="Tienda Content Management System - v'.expVersion::getVersion(true).'" '.XHTML_CLOSING.'>' . "\n";
...
//					echo "\t".'<link rel="alternate" type="application/rss+xml" title="' . $title . '" href="' . expCore::makeRSSLink($params) . "\" />\n";
	...
	public static function satisfyThemeRequirements() {
		...
		if ($validateTheme['headerinfo']==false) {
			echo "<h1 style='padding:10px;border:5px solid #992222;color:red;background:white;position:absolute;top:100px;left:300px;width:400px;z-index:999'>expTheme::head() is a required function in your theme.  Please refer to the Tienda documentation for details:<br />
		...
		if ($validateTheme['footerinfo']==false) {
			echo "<h1 style='padding:10px;border:5px solid #992222;color:red;background:white;position:absolute;top:100px;left:300px;width:400px;z-index:999'>expTheme::foot() is a required function in your theme.  Please refer to the Tienda documentation for details:<br />
...
//		$mobile = self::is_mobile();
		...
		if (self::inAction() && (!empty($action_maps[$module]) && (array_key_exists($_REQUEST['action'], $action_maps[$module]) || array_key_exists('*', $action_maps[$module])))) {
			...
			// since this will be the second time Tienda calls this function on the page load.
