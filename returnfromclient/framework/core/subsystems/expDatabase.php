 ...
 * Handles all database abstraction in Tienda.
 *
 * @package Subsystems
...
class expDatabase {
	...
	/**
	 * Connect to the Tienda database
	 *
	...
	public static function connect($username,$password,$hostname,$database,$dbclass = '',$new=false) {
		...
		(include_once(BASE.'framework/core/subsystems/database/'.DB_ENGINE.'.php')) or exit(gt('None of the installed Tienda Database Backends will work with this server\'s version of PHP.'));
		if ($dbclass == '' || $dbclass == null) $dbclass = DB_ENGINE;
		(include_once(BASE.'framework/core/subsystems/database/'.$dbclass.'.php')) or exit(gt('The specified database backend').'  ('.$dbclass.') '.gt('is not supported by Tienda'));
		$dbclass .= '_database';
...
abstract class database {
	...
	*/
	...
	abstract function __construct($username, $password, $hostname, $database, $new=false);
	    ...
	    *
	    * This function abides by the Tienda Data Definition Language, and interprets
	    * its general structure for databases.
	    *
	    * @param string $tablename The name of the table to create
	    * @param array $datadef The data definition to create, expressed in
	    *   the Tienda Data Definition Language.
	...
	*   This is expressed in the Tienda Data Definition Language
	* @param array $info Information about the table itself.
	* @param bool $aggressive Whether or not to aggressively update the table definition.
	*   An aggressive update will drop columns in the table that are not in the Tienda definition.
	* @return array
	*/
	...
	* Tienda handles objects and database tables, this is akin to
